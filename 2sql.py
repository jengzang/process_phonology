...
from match_fromdb import get_tsvs
import pandas as pd
import sqlite3
...
def build_dialect_database(
        han_filename="漢字音典字表檔案（長期更新）.xlsx",
        other_filename="Dialect_files.xlsx",
        db_filename="dialects_query.db",
        source_dir="source",
        output_dir="data"
):
    source_path = Path(source_dir)
    han_file = source_path / han_filename
    other_file = source_path / other_filename

    # 確保輸出資料夾存在
    output_path = Path(output_dir)
    output_path.mkdir(parents=True, exist_ok=True)
    sqlite_db = output_path / db_filename
    ...
    print(f"✅ SQLite 資料庫 `{db_filename}` 已建立，dialects 表已更新完成。")
...
def sync_dialects_flags(all_db_path="data/dialects_all.db",
                        query_db_path="data/dialects_query.db",
                        log_path="data/dialects_all_log.txt"):
...
def process_phonology_excel(
    excel_file="source/聲韻.xlsx",
    sheet_name="層級",
    db_file="data/characters.db",
    log_file="data/dialects_all_log.txt"
):
...
def write_to_sql():
    ...
    db_path = os.path.join(os.getcwd(), "data/dialects_all.db")
    print("🚀 開始導入資料...")
    process_all2sql(tsv_paths, db_path)
    print("开始处理重复行以及标记多音字")
    process_polyphonic_annotations("data/dialects_all.db")
    print("开始寫入存儲標記")
    sync_dialects_flags()
