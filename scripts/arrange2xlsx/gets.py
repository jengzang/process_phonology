...
from common.constants import vowel_pattern, TONE_MAP
...
def get_vowels_from_tsv(tsv_file_path: str, char_list: list) -> pd.DataFrame:
    ...
    if '#漢字' not in tsv_df.columns:
        ...
        # ✅ 移除 '#漢字' 或 '音標' 缺失或為 "0" 的行
    tsv_df = tsv_df[
        tsv_df['#漢字'].notna() &
        tsv_df['音標'].notna() &
        (tsv_df['#漢字'].astype(str).str.strip() != "0") &
        (tsv_df['音標'].astype(str).str.strip() != "0")
        ]

    if char_list == "all":
        char_list = tsv_df['#漢字'].drop_duplicates().tolist()
        char_list = [char for char in char_list if not re.search(r"[□■⬜⬛☐☑☒▯▢▣█�]", char)]
    else:
...
def get_notes_from_tsv(tsv_file_path: str, char_list: list) -> pd.DataFrame:
    # 讀取 TSV，第一列是標題但含 #
    df = pd.read_csv(tsv_file_path, sep="\t", dtype=str)
    df.columns = [col.lstrip("#").strip() for col in df.columns]

    # 保證欄位存在
    assert "漢字" in df.columns, "缺少「漢字」欄位"
    assert "解釋" in df.columns, "缺少「解釋」欄位"

    # 清理空值
    df = df.fillna("")
    df["漢字"] = df["漢字"].astype(str).str.strip()
    df["解釋"] = df["解釋"].astype(str).str.strip()

    if char_list != "all":
        df = df[df["漢字"].isin(char_list)]

    # 統一欄名
    df.rename(columns={"漢字": "汉字", "解釋": "註釋"}, inplace=True)

    return df[["汉字", "註釋"]].copy()


# # # 本地路径与查询字列表
tsv_file = r"C:\Users\joengzaang\myfiles\杂文件\声韵处理\output\東莞東坑.tsv"
# tsv_file = r"C:\Users\joengzaang\PycharmProjects\process_phonology\processed\東莞莞城.tsv"
# char_list = ["彷", "後", "忽"]
char_list = "all"
#
# # 调用函数并打印结果
result = get_notes_from_tsv(tsv_file, char_list)
