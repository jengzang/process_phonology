...
import re
import pandas as pd
from pypinyin.style import tone


def extract_tone_maps(shortname: str, dialect_excel="source/Dialect_files.xlsx"):
    tone_shu = {}  # 陰平 ~ 陽去（[1] ~ [6]）
    tone_ru = {}  # 陰入、陽入（[7], [8]）
...
def convert_tones(tone: dict, shortname: str):
    tsv_file_path = os.path.join("output", f"{shortname}.tsv")
    if not os.path.exists(tsv_file_path):
        print(f"错误：找不到文件 {tsv_file_path}")
        with open("data/debug.txt", "a", encoding="utf-8") as f:
...
def tone_jyut2yindian(shortname: str):
    ...
    if error_logs:
        ...
        with open("data/debug.txt", "a", encoding="utf-8") as f:
            f.write("\n".join(error_logs))
        print(f"⚠️ 錯誤紀錄已寫入：data/debug.txt（共 {len(error_logs)} 條）")
